...
template <> struct Merge<Ints<>, Ints<>> { typedef Ints<> type; };
...
struct MergeHelper<true, Ints<A, Vals1...>, Ints<B, Vals2...>>
    : public Concat<Ints<B>,
                    typename Merge<Ints<A, Vals1...>, Ints<Vals2...>>::type> {};

template <typename Ints1> struct SortHelper;

template <> struct SortHelper<Ints<>> { typedef Ints<> type; };

template <int A> struct SortHelper<Ints<A>> { typedef Ints<A> type; };
...
template <int... Vals> struct Sort : public SortHelper<Ints<Vals...>> {};
...
int main() {
    ...
    cout << typeid(
                Sort<11, 13, 5, 79, 73, 47, 31, 89, 2, 97, 41, 11, 61, 19, 29,
                     29, 59, 67, 89, 53, 2, 37, 43, 17, 47, 89, 79, 23, 61, 83,
                     7, 83, 11, 3, 13, 3, 11, 59, 71, 37, 29, 19, 83>::type)
                .name()
         << endl;
